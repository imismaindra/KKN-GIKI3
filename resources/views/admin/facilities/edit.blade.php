...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.facilities.update', $facility->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div>
            ...
            <input type="hidden" name="icon" id="icon-input" value="{{ old('icon', $facility->icon) }}">
            ...
            <div class="flex items-center space-x-4">
                <div id="icon-preview-wrapper" class="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-700 shadow-inner">
                    <span id="selected-icon-display" class="material-symbols-outlined text-3xl">{{ old('icon', $facility->icon ?? 'help_outline') }}</span>
                </div>
                ...
                <div class="flex-1 relative">
                    <button type="button" onclick="toggleIconPicker()" class="w-full flex items-center justify-between px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-700 text-left transition duration-150">
                        <span id="selected-icon-text" class="truncate font-medium">{{ old('icon', $facility->icon) ? 'Ikon terpilih: ' . old('icon', $facility->icon) : 'Pilih dari daftar ikon...' }}</span>
        ...
        <div>
            ...
            <div id="image-dropzone" class="border-2 border-dashed border-slate-200 hover:border-blue-500 rounded-2xl p-6 bg-slate-50 transition cursor-pointer relative flex flex-col items-center justify-center text-center group min-h-[160px]">
                ...
                <div id="image-placeholder-wrapper" class="{{ $facility->image_path ? 'hidden' : '' }} space-y-2">
                ...
                <div id="image-preview-container" class="{{ $facility->image_path ? '' : 'hidden' }} w-full max-w-md aspect-video rounded-xl overflow-hidden relative border border-slate-200">
                    <img id="image-preview-el" src="{{ $facility->image_path ? Storage::url($facility->image_path) : '' }}" class="w-full h-full object-cover">
