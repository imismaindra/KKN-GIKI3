...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.facilities.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
        @csrf
        
        <div>
            <label for="name" class="block text-sm font-semibold text-slate-700 mb-1">Nama Fasilitas</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required autofocus placeholder="Contoh: Laboratorium Komputer"
        ...
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Pilih Ikon Fasilitas (Opsional)</label>
            ...
            <div class="flex items-center space-x-4">
                <div id="icon-preview-wrapper" class="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-700 shadow-inner">
                    <span id="selected-icon-display" class="material-symbols-outlined text-3xl">{{ old('icon', 'help_outline') }}</span>
                </div>
                ...
                <div class="flex-1 relative">
                    <button type="button" onclick="toggleIconPicker()" class="w-full flex items-center justify-between px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-700 text-left transition duration-150">
                        <span id="selected-icon-text" class="truncate font-medium">{{ old('icon') ? 'Ikon terpilih: ' . old('icon') : 'Pilih dari daftar ikon...' }}</span>
            ...
            <div id="icon-picker-panel" class="hidden mt-3 p-4 bg-white border border-slate-100 rounded-2xl shadow-xl space-y-4 max-h-80 overflow-y-auto z-20 relative">
                <div class="relative">
                    <input type="text" id="icon-search-input" onkeyup="filterIcons()" placeholder="Cari nama ikon (contoh: buku, kelas, olahraga)..."
                ...
                <div class="grid grid-cols-4 sm:grid-cols-6 gap-2" id="icon-grid">
                    ...
                    <button type="button" onclick="selectIcon('code', 'RPL')" data-name="code rpl coding pemrograman komputasi" class="icon-option-btn p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 flex flex-col items-center gap-1 group transition">
                    ...
                    <button type="button" onclick="selectIcon('engineering', 'Bengkel')" data-name="engineering bengkel mesin teknik perkakas" class="icon-option-btn p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 flex flex-col items-center gap-1 group transition">
                        <span class="material-symbols-outlined text-2xl text-slate-600 group-hover:text-blue-600">engineering</span>
                        <span class="text-[10px] text-slate-400 group-hover:text-blue-500 text-center truncate w-full">Bengkel</span>
                    </button>
                    <button type="button" onclick="selectIcon('palette', 'Seni')" data-name="palette seni lukis multimedia desain gambar" class="icon-option-btn p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 flex flex-col items-center gap-1 group transition">
                        <span class="material-symbols-outlined text-2xl text-slate-600 group-hover:text-blue-600">palette</span>
                        <span class="text-[10px] text-slate-400 group-hover:text-blue-500 text-center truncate w-full">Desain</span>
                    </button>
                    <button type="button" onclick="selectIcon('theater_comedy', 'Aula')" data-name="theater_comedy aula panggung auditorium seni tari" class="icon-option-btn p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 flex flex-col items-center gap-1 group transition">
        ...
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Gambar Fasilitas (Opsional)</label>
            ...
            <div id="image-dropzone" class="border-2 border-dashed border-slate-200 hover:border-blue-500 rounded-2xl p-6 bg-slate-50 transition cursor-pointer relative flex flex-col items-center justify-center text-center group min-h-[160px]">
                ...
                <div id="image-preview-container" class="hidden w-full max-w-md aspect-video rounded-xl overflow-hidden relative border border-slate-200">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" required placeholder="Jelaskan mengenai fasilitas ini, spesifikasi, kegunaan, dll."
