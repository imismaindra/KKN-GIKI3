...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.majors.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
        @csrf
        
        <div>
            <label for="name" class="block text-sm font-semibold text-slate-700 mb-1">Nama Jurusan / Program Keahlian</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required autofocus placeholder="Contoh: Rekayasa Perangkat Lunak"
        ...
        <div>
            ...
            <input type="text" name="icon" id="icon" value="{{ old('icon') }}" placeholder="Contoh: academic-cap"
                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-800 transition duration-150">
            <p class="text-xs text-slate-400 mt-1">Nama class atau nama ikon pustaka yang digunakan (opsional).</p>
            @error('icon') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="image_path" class="block text-sm font-semibold text-slate-700 mb-1">Pilih Gambar Sampul Jurusan (Opsional)</label>
            ...
            <p class="text-xs text-slate-400 mt-1">Gambar dokumentasi jurusan. Maksimal 2MB.</p>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" required placeholder="Jelaskan mengenai jurusan ini, materi pelajaran yang dipelajari, prospek kerja, dll."
