...
@section('page_title', 'Tambah Guru/Staf Baru')
...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.teachers.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
        @csrf
        
        <div>
            <label for="name" class="block text-sm font-semibold text-slate-700 mb-1">Nama Lengkap beserta Gelar</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required autofocus placeholder="Contoh: Dra. Hajah Siti Aminah, M.Pd."
